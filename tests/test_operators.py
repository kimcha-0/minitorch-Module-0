...
def test_same_as_python(x: float, y: float) -> None:
    """Check that the main operators all return the same value of the python
    version"""
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert_close(minitorch.operators.sum(ls1) + minitorch.operators.sum(ls2), minitorch.operators.sum(zipWith(ls1, ls2, add)))
